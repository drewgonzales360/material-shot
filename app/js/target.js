...
function startGame() {
  ...
  diffCode = difficulty.diffCode;
  document.getElementById("time").innerHTML = "Start Game.";
  document.getElementById('score').innerHTML = "Score: " + score;
  document.getElementById('high-score').innerHTML = "High Score: " + difficulty.highScore;
  let game = setInterval(function(){
    let x = Math.floor(Math.random()*ctx.canvas.width);
    let y = Math.floor(Math.random()*ctx.canvas.height)
    drawTarget(x, y, difficulty.radius * (Math.random()+1), difficulty.shrinkFactor);
  }, (difficulty.targetSpawnRate*(Math.random())));
  score = 0;
  displayTime(gameTime, game);
}
...
document.getElementById('quit').addEventListener('click', function(){
  ipcRenderer.send('save-score', diffCode, score);
